...
@Inject constructor(private val repository: IRepository) :
    ...
    private val _githubProjects: MutableLiveData<Event<Resource<List<Recipe>>>> =
        MutableLiveData()
    override val githubProjects: LiveData<Event<Resource<List<Recipe>>>>
        get() = _githubProjects

    init {
        viewModelScope.launch {
            repository.getRecipes("chicken")
                .onEach { _githubProjects.value = Event(it) }
